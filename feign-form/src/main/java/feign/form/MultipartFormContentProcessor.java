...
public class MultipartFormContentProcessor implements ContentProcessor {
	 ...
	 *
	 * @param delegate
	 *            specific delegate encoder for cases, when this processor couldn't
	...
	public void process(RequestTemplate template, Charset charset, Map<String, Object> data) throws EncodeException {
		...
		try (val output = new Output(charset)) {
			...
			val contentTypeHeaderValue = new StringBuilder().append(getSupportedContentType().getHeader())
					.append("; charset=").append(charset.name()).append("; boundary=").append(boundary).toString();
