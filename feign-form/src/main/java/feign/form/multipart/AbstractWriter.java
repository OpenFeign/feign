...
public abstract class AbstractWriter implements Writer {
	 ...
	 *            file name.
	 * @param contentType
	 *            type of file content. May be the {@code null}, in that case it
	 *            will be determined by file name.
	 */
	protected void writeFileMetadata(Output output, String name, String fileName, String contentType) {
		val contentDespositionBuilder = new StringBuilder().append("Content-Disposition: form-data; name=\"")
				.append(name).append("\"");
		if (fileName != null) {
		...
		val string = new StringBuilder().append(contentDespositionBuilder.toString()).append(CRLF)
				.append("Content-Type: ").append(fileContentType).append(CRLF)
				.append("Content-Transfer-Encoding: binary").append(CRLF).append(CRLF).toString();
