...
public class Server {
	...
	public String upload1(@PathVariable("folder") String folder, @RequestPart("file") MultipartFile file,
	...
	public String upload2(@RequestBody MultipartFile file, @PathVariable("folder") String folder,
			@RequestParam(value = "message", required = false) String message) throws IOException {
		return new String(file.getBytes()) + ':' + message + ':' + folder;
	}

	@PostMapping(path = "/multipart/upload3/{folder}", consumes = MULTIPART_FORM_DATA_VALUE)
	public String upload3(@RequestBody MultipartFile file, @PathVariable("folder") String folder,
			@RequestParam(value = "message", required = false) String message) {
		return file.getOriginalFilename() + ':' + file.getContentType() + ':' + folder;
	}

	@PostMapping("/multipart/upload4/{id}")
	public String upload4(@PathVariable("id") String id, @RequestBody Map<String, Object> map,
	...
	public ResponseEntity<String> upload6(@RequestParam("popa1") MultipartFile popa1,
			@RequestParam("popa2") MultipartFile popa2) throws Exception {
