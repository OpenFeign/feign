...
/**
 * Implementation of {@link HttpMessageConverter} that can read
 * multipart/form-data HTTP bodies (writing is not handled because that is
 * already supported by {@link FormHttpMessageConverter}).
 * <p>
...
public class SpringManyMultipartFilesReader extends AbstractHttpMessageConverter<MultipartFile[]> {
	...
	/**
	 * Construct an {@code AbstractHttpMessageConverter} that can read
	 * mulitpart/form-data.
	 *
	 * @param bufSize
	 *            The size of the buffer (in bytes) to read the HTTP multipart body.
	 */
	...
	private ByteArrayMultipartFile readMultiPart(MultipartStream multipartStream) throws IOException {
		val multiPartHeaders = splitIntoKeyValuePairs(multipartStream.readHeaders(), NEWLINES_PATTERN, COLON_PATTERN,
				false);

		val contentDisposition = splitIntoKeyValuePairs(multiPartHeaders.get(CONTENT_DISPOSITION), SEMICOLON_PATTERN,
				EQUALITY_SIGN_PATTERN, true);
		...
		return new ByteArrayMultipartFile(contentDisposition.get("name"), contentDisposition.get("filename"),
				multiPartHeaders.get(CONTENT_TYPE), bodyStream.toByteArray());
	}
