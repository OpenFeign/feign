...
public class JSONObject {
  ...
  public <E extends Enum<E>> E getEnum(Class<E> clazz, String key) throws JSONException {
    ...
    if (val == null) {
      ...
      throw wrongValueFormatException(key, "enum of type " + quote(clazz.getSimpleName()), opt(key),
          null);
    }
    return val;
  }
   ...
   *        converted exactly. When <code>false</code>, they will be converted to {@link String}
   ...
   *
   * If either of the numbers are Double or Float instances, then they are checked to have a finite
   * value. If either value is not finite (NaN or &#177;infinity), then this function will always
   * return false. If both numbers are finite, they are first checked to be the same type and
   ...
   * converted to {@link BigDecimal}s. Finally the BigDecimal values are compared using
   ...
   * is an array or collection, wrap it in a JSONArray. If it is a map, wrap it in a JSONObject. If
   * it is a standard property (Double, String, et al) then it is already wrapped. Otherwise, if it
