...
public class RecordInvokeUtils {
  ...
  private static final MethodHandle MH_GET_ANNOTATIONS;
  ...
  static {
    ...
    MethodHandle MH_getAnnotations;
    ...
    try {
      ...
      MH_getAnnotations = LOOKUP.findVirtual(c, "getAnnotations", methodType(Annotation[].class))
    ...
    MH_GET_ANNOTATIONS = MH_getAnnotations;
  }
    ...
    try {
      ...
      for (int i = 0; i < rawComponents.length; i++) {
        ...
        recordComponents[i] = new RecComponent((String) MH_GET_NAME.invokeExact(comp),
            (Type) MH_GET_TYPE.invokeExact(comp), i,
            (Annotation[]) MH_GET_ANNOTATIONS.invokeExact(comp));
      }
      if (comparator != null)
