...
public class InputStreamAndFileEncoderTest {
  ...
  void streamingRequestCanRetry() throws Exception {
    ...
    LargeStreamTestInterface api =
        createRetryableFeignBuilder()
            .target(LargeStreamTestInterface.class, "http://localhost:" + server.getPort());

    byte[] requestData = new byte[16184];
    new Random().nextBytes(requestData);

    // if we use a plain ByteArrayInputStream, then it is infinitely resettable, so the following
    // will retry
    // if instead we wrapped this in a reset limited inputstream (like a BufferedInputStream with
    // size set to 1024), retry would fail.
    // As of now, I don't see a way to tell the server to read *some* of the request and then fail,
    // so we aren't getting good test coverage of the partial request reset scenario
  ...
  void streamingRequestRetryFailsIfTooMuchDataRead() throws Exception {
    ...
    // if we wrap the inputstream in a reset limited inputstream like a BufferedInputStream, retry
    // will fail
    // As of now, I don't see a way to tell the server to read *some* of the request and then fail,
    // so we aren't getting good test coverage of the partial request reset scenario
    FeignException e =
        assertThrows(
            ...
            () ->
                api.postLargeStream(
                    new BufferedInputStream(new ByteArrayInputStream(requestData), 1024)));
