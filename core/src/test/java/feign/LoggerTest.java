...
public class LoggerTest {
  ...
  public void levelEmits(final Logger.Level logLevel, List<String> expectedMessages)
    ...
    try {
      SendsStuff api =
          Feign.create(
              SendsStuff.class,
              "http://localhost:" + server.getUrl("").getPort(),
  ...
  static @dagger.Module(injects = Feign.class, overrides = true, addsTo = Feign.Defaults.class)
  class DefaultModule {
    final Logger logger;
  ...
  static class LessReadTimeoutModule {
    @Provides
    Request.Options lessReadTimeout() {
      return new Request.Options(10 * 1000, 50);
    }
  }

  @Test(dataProvider = "levelToReadTimeoutOutput")
  public void readTimeoutEmits(final Logger.Level logLevel, List<String> expectedMessages)
      throws IOException, InterruptedException {
    final MockWebServer server = new MockWebServer();
  ...
  public void unknownHostEmits(final Logger.Level logLevel, List<String> expectedMessages)
      throws IOException, InterruptedException {
    ...
    try {
      SendsStuff api =
          Feign.create(
              ...
              "http://robofu.abc",
              new RetryOnceModule(),
              new DefaultModule(logger, Logger.Level.BASIC));
