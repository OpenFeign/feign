...
final class TrustingSSLSocketFactory extends SSLSocketFactory
  ...
  private static LoadingCache<String, SSLSocketFactory> sslSocketFactories =
      CacheBuilder.newBuilder()
          .build(
              new CacheLoader<String, SSLSocketFactory>() {
                @Override
  ...
  private TrustingSSLSocketFactory(String serverAlias) {
    ...
    if (serverAlias.isEmpty()) {
      ...
      try {
        KeyStore keyStore =
            loadKeyStore(Resources.newInputStreamSupplier(Resources.getResource("keystore.jks")));
  ...
  private static KeyStore loadKeyStore(InputSupplier<InputStream> inputStreamSupplier)
      throws IOException {
