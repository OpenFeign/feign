...
final class MethodHandler {
  ...
  public Object executeAndDecode(
      String configKey, RequestTemplate template, TypeToken<?> returnType) throws Throwable {
    ...
    try {
      ...
      if (response.status() >= 200 && response.status() < 300) {
        if (returnType.getRawType().equals(Response.class)) {
          ...
          if (!location.isEmpty()) return URI.create(location.get(0));
