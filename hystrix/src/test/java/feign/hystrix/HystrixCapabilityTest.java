...
public class HystrixCapabilityTest extends HystrixBuilderTest {
  ...
  protected <E> E target(Class<E> api, String url) {
    return Feign.builder().addCapability(new HystrixCapability()).target(api, url);
  }

  @Override
  protected <E> E target(Target<E> api) {
    return Feign.builder().addCapability(new HystrixCapability()).target(api);
  }

  @Override
  protected <E> E target(Class<E> api, String url, E fallback) {
    return Feign.builder()
        .addCapability(new HystrixCapability().fallback(api, fallback))
        .target(api, url);
  }
  ...
  protected TestInterface target() {
    return Feign.builder()
        .addCapability(
            new HystrixCapability().fallback(TestInterface.class, new FallbackTestInterface()))
