...
public class AnnotationErrorDecoderPriorityTest
    extends AbstractAnnotationErrorDecoderTest<
        AnnotationErrorDecoderPriorityTest.TestClientInterfaceWithExceptionPriority> {
  ...
  @ErrorHandling(
      codeSpecific = {
        @ErrorCodes(
            codes = {404},
            generate = ClassLevelNotFoundException.class),
        @ErrorCodes(
            codes = {403},
            generate = UnauthenticatedOrUnauthorizedException.class)
      },
  ...
  interface TestClientInterfaceWithExceptionPriority {
    @ErrorHandling(
        codeSpecific = {
          @ErrorCodes(
              codes = {404},
              generate = Method1NotFoundException.class),
          @ErrorCodes(
              codes = {401},
              generate = UnauthenticatedOrUnauthorizedException.class)
        },
    ...
    @ErrorHandling(
        codeSpecific = {
          @ErrorCodes(
              codes = {404},
              generate = Method2NotFoundException.class),
          @ErrorCodes(
              codes = {500, 503},
              generate = ServeErrorException.class)
        })
    void method2Test();
  ...
  public void initAnnotationErrorDecoderPriorityTest(
      String testType,
      int errorCode,
      String method,
      Class<? extends Exception> expectedExceptionClass) {
